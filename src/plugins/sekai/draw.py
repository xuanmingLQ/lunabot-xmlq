...
# 绘制Sekai默认背景函数（将绘图下放到Painter中）
def draw_sekai_bg(time_color: bool, main_hue: float, size_fixed_rate: float, p: Painter):
    w = p.w
    h = p.h
    ...
    if time_color:
        ...
        ms = 1.0
        ml = 1.0
    size_fixed_rate = size_fixed_rate
...
class SekaiBg(WidgetBg):
    ...
    def draw(self, p: Painter):
        # p.add_operation(draw_sekai_bg, self.time_color, self.main_hue, self.size_fixed_rate)
        draw_sekai_bg(self.time_color, self.main_hue, self.size_fixed_rate, p)
